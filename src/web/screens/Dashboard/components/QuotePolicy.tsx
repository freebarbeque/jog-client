...
import {
    ...
    getUserAddress,
    getDriversList,
} from '~/common/selectors/userDetils';
import {
    getPolicyQuoteRequest,
    getLoadingState,
    getStartPolicyDate,
} from '~/common/selectors/policyQuoteRequest';
...
} from '~/common/actions/policyQuoteRequest';
...
class QuotePolicy extends React.PureComponent<IQuotePolicyProps, {}> {
                ...
                <QuoteLoadingWrapper>
                    <Container>
                        ...
                        <Main>
                            ...
                            <QuoteLoadingFooter>
                                Please wait while we find you the best quotes on
                                the market...
                            </QuoteLoadingFooter>
                        </Main>
                    </Container>
    ...
    handleDatePickerChange = (value: any) => {
        this.props.updateStartDateOnPolicyQuoteRequest(
            this.props.motorId,
            value
        );
    };
    ...
    getDriversNames = (selected, driversList) => {
        ...
        output = array.reduce((acc, cur, index, array) => {
            ...
            if (cur.first_name && cur.last_name) {
                acc += `${cur.first_name} ${cur.last_name}${
                    index < array.length - 1 ? suffix : ``
                }`;
            }
            return acc;
        }, '');
    ...
    render() {
        const {
            policyQuoteRequest: { vehicle, driver, address },
            startDate,
            isLoading,
        } = this.props;
        const { drivers: driversList } = this.props;
        const driversNames: string =
            driver && driver.length > 0 && driversList && driversList.length > 0
            ...
            <div className={this.props.className}>
                ...
                <QuoteContentContainer>
                    <QuoteField
                        ...
                        onClick={() =>
                            this.props.push(
                                `/app/motor/${this.props.motorId}/quote/vehicle`
                            )
                        }
                        completed={!!vehicle}
                    />
                    <QuoteField
                        ...
                        onClick={() =>
                            this.props.push(
                                `/app/user/motor/${this.props.motorId}/holder`
                            )
                        }
                    ...
                    <QuoteField
                        ...
                        onClick={() =>
                            this.props.push(
                                `/app/motor/${this.props.motorId}/quote/address`
                            )
                        }
